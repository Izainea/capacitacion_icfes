# ==============================================================================
# APLICACIÓN SHINY: VERSIÓN FINAL CON GESTIÓN SEGURA DE CREDENCIALES
# ==============================================================================
# Este script implementa la gestión de secretos a través de variables de
# entorno para realizar llamadas autenticadas a la API de Socrata.
# ------------------------------------------------------------------------------
...
# --- GESTIÓN SEGURA DEL APP TOKEN DE SOCRATA ---
socrata_app_token <- Sys.getenv("SOCRATA_APP_TOKEN")
if (socrata_app_token == "") {
    warning("ADVERTENCIA: La variable de entorno 'SOCRATA_APP_TOKEN' no está configurada. Las llamadas a la API pueden ser limitadas. Consulte la lección 3 para configurarla.")
}
    ...
    dashboardBody(
        ...
        tabItems(
            ...
            tabItem(tabName = "monitoring",
                ...
                p("Esta herramienta ejecuta una tarea idéntica en las tres fuentes: obtener los primeros 10,000 registros de BOGOTA D.C. y calcular su puntaje promedio, midiendo el tiempo total de la operación."),
                fluidRow(
                    box(
...
server <- function(input, output, session) {
        ...
        withProgress(message = 'Consultando API Socrata...', value = 0.7, {
            ...
            tryCatch({
                req_api <- request(api_base_url) %>% 
                           req_headers("X-App-Token" = socrata_app_token) %>%
                           req_url_query(
                               `$limit` = 10000,
                ...
                datos_api <- as_tibble(resp_body_json(resp_api, simplifyVector = TRUE))
    ...
    output$time_flat_file <- renderValueBox({
      valueBox(
        if (is.null(benchmark_results$time_flat)) "N/A" else paste(round(benchmark_results$time_flat, 4), "s"),
        "Tiempo (Archivo Plano)",
        icon = icon("file-alt"),
        color = "red"
      )
    })
    ...
    output$time_db <- renderValueBox({
      valueBox(
        if (is.null(benchmark_results$time_db)) "N/A" else paste(round(benchmark_results$time_db, 4), "s"),
    ...
    output$time_api <- renderValueBox({
      valueBox(
        if (is.null(benchmark_results$time_api)) "N/A" else paste(round(benchmark_results$time_api, 4), "s"),
        "Tiempo (API)",
        icon = icon("cloud"),
        color = "green"
      )
    ...
    observe({
        ...
        tryCatch({
            # --- Obtener periodos distintos (SEGURO) ---
            query_periodos <- "SELECT DISTINCT periodo ORDER BY periodo DESC"
            req_p <- request(api_base_url) %>% 
                     req_headers("X-App-Token" = socrata_app_token) %>%
                     req_url_query(`$query` = query_periodos)
            ...
            # --- Obtener departamentos distintos (SEGURO) ---
            query_deptos <- "SELECT DISTINCT cole_depto_ubicacion ORDER BY cole_depto_ubicacion"
            req_d <- request(api_base_url) %>% 
                     req_headers("X-App-Token" = socrata_app_token) %>%
                     req_url_query(`$query` = query_deptos)
    ...
    api_data <- eventReactive(input$run_api_query, {
        ...
        withProgress(message = 'Consultando API de Datos Abiertos...', value = 0.3, {
            tryCatch({
                # --- Construcción de consulta dinámica (SEGURO) ---
